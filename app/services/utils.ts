...
export class CRUDService extends ReadOnlyService {
    async create<T = any>(data: T, params: DefaultQueryParams | null = null, url_params: Record<string, string | number> = {}): Promise<T> {
        const response = await this.$api.post(`${this.getUrl(url_params)}/`, data, {params})
        return response.data
    }

    async update<T = any>(id: number | string, data: T, params: DefaultQueryParams | null = null, url_params: Record<string, string | number> = {}): Promise<T> {
        const url = this.getUrl(url_params)
        const response = await this.$api.patch(`${url}/${id}/`, data, {params})
        return response.data
    }

    async delete<T = any>(id: number | string, params: DefaultQueryParams | null = null, url_params: Record<string, string | number> = {}): Promise<T> {
        const url = this.getUrl(url_params)
